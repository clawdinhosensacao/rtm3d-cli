...
void step_fd3d(const Volume3D& vel, const std::vector<float>& damp, float dt, float dx,
               float dy, float dz, const std::vector<float>& prev,
               const std::vector<float>& cur, std::vector<float>& nxt) {
  const auto nx = vel.nx();
  const auto ny = vel.ny();
  const auto nz = vel.nz();

  std::fill(nxt.begin(), nxt.end(), 0.0f);

  for (std::size_t iz = 1; iz + 1 < nz; ++iz) {
    for (std::size_t iy = 1; iy + 1 < ny; ++iy) {
      for (std::size_t ix = 1; ix + 1 < nx; ++ix) {
        const auto i = vel.index(ix, iy, iz);
        const float d2x = (cur[vel.index(ix + 1, iy, iz)] - 2.0f * cur[i] +
                           cur[vel.index(ix - 1, iy, iz)]) /
                          (dx * dx);
        const float d2y = (cur[vel.index(ix, iy + 1, iz)] - 2.0f * cur[i] +
                           cur[vel.index(ix, iy - 1, iz)]) /
                          (dy * dy);
        const float d2z = (cur[vel.index(ix, iy, iz + 1)] - 2.0f * cur[i] +
                           cur[vel.index(ix, iy, iz - 1)]) /
                          (dz * dz);
  ...
  for (std::size_t iz = 0; iz < nz; ++iz) {
    for (std::size_t iy = 0; iy < ny; ++iy) {
      for (std::size_t ix = 0; ix < nx; ++ix) {
        const auto dist_x = std::min(ix, nx - 1 - ix);
        const auto dist_y = std::min(iy, ny - 1 - iy);
        const auto dist_z = std::min(iz, nz - 1 - iz);
        const auto dist = std::min({dist_x, dist_y, dist_z});
        float coeff = 1.0f;
        if (dist < pml) {
...
std::vector<float> ricker_wavelet(std::size_t nt, float dt, float f0) {
  ...
  for (std::size_t it = 0; it < nt; ++it) {
    ...
    const float a = static_cast<float>(M_PI) * f0 * t;
    const float a2 = a * a;
...
MigrationResult run_single_shot_rtm(const Marmousi2D& model, const RtmConfig& cfg) {
  ...
  for (std::size_t iz = 0; iz < model.nz; ++iz) {
    for (std::size_t iy = 0; iy < cfg.ny; ++iy) {
      for (std::size_t ix = 0; ix < model.nx; ++ix) {
        vel(ix, iy, iz) = model.v[iz * model.nx + ix];
      }
    }
  }

  const auto n = vel.size();
  auto damp = make_damp(vel.nx(), vel.ny(), vel.nz(), cfg.pml);
  ...
  // Forward source + synthetic acquisition
  ...
  // Backward receiver propagation + imaging condition
  ...
  for (std::size_t iz = 0; iz < out.nz; ++iz) {
    for (std::size_t ix = 0; ix < out.nx; ++ix) {
      out.inline_xz[iz * out.nx + ix] = image[vel.index(ix, ymid, iz)];
    }
  }
  return out;
}
